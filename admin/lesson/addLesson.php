...
if (isset($_POST['save'])) {
    ...
    $stmt = $pdo->prepare("INSERT INTO lessons (title, category) VALUES (?, ?)");
    $stmt->execute([$lessonTitle, $category]);
    $lessonId = $pdo->lastInsertId();

    // Insert contents and page numbers into the database
    foreach ($contents as $pageNumber => $content) {
        $page = $pageNumber + 1;
        $stmt = $pdo->prepare("INSERT INTO pages (lesson_id, content, page_number) VALUES (?, ?, ?)");
            ...
            <div class="col py-3 bg-light" id="m-contents">
                ...
                <div class="lessons">
                    ...
                    <form method="post">
                        ...
                        <label for="category">Lesson Category</label> <br>
