...
// Fetch lesson title
$stmt = $pdo->prepare("SELECT title FROM tbllesson WHERE LessonID = ?");
$stmt->execute([$lessonId]);
$lessonTitle = $stmt->fetchColumn();

// Fetch lesson pages
$pagesStmt = $pdo->prepare("SELECT * FROM tblpages WHERE lesson_id = ? ORDER BY page_number");
...
if ($current_page < $total_pages) {
    ...
    echo '<a class="complete-button" href="../../teacher/dashboard.php">Complete</a>';
}
echo '</div>';
